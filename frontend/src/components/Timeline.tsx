...
export function Timeline({ events, timeframeHours, alertingSessionIds }: TimelineProps) {
  ...
  const appGroups = useMemo(() => {
    ...
    sessionGroups.forEach((sessionEvents, sessionId) => {
      // Use the earliest event's source_app — the most recently arrived event may have an
      // incorrect source_app (e.g. AssistantMessage/TokenUsage events generated by another
      // project's hooks with a hardcoded source_app value).
      const earliestEvent = [...sessionEvents].sort((a, b) => a.timestamp - b.timestamp)[0]
      const appName = earliestEvent?.source_app || 'Unknown App'
